...
describe('Modal', () => {
  ...
  it('calls onOpenChange when closed via Close button', async () => {
    ...
    render(
      <Modal open onOpenChange={onOpenChange} title="Close test" showCloseIcon={true}>
        <div>Body</div>
      </Modal>
    )
  ...
  it('calls onOpenChange when clicking on overlay/backdrop', async () => {
    ...
    const overlay = document.body.querySelector('.ohs-modal-overlay') as HTMLElement
  ...
  it('exposes description via aria-describedby and renders the description visibly', async () => {
    render(
      <Modal open id="test-modal" title="Desc test" description={<span>Visible desc</span>}>
        <div>Body</div>
      </Modal>
    )

    const content = document.body.querySelector('.ohs-modal-content') as HTMLElement
